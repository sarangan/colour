	...
	public function index() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
			if (!isset($this->request->get['module_id'])) {
				$this->model_extension_module->addModule('mpgallery', $this->request->post);
			} else {
		...
		$data['help_gallery'] = $this->language->get('help_gallery');
		...
		if (!isset($this->request->get['module_id'])) {
			$data['breadcrumbs'][] = array(
				...
				'href' => $this->url->link('extension/module/mpgallery', 'token=' . $this->session->data['token'], true)
			);
		} else {
			$data['breadcrumbs'][] = array(
				'text' => $this->language->get('heading_title'),
				'href' => $this->url->link('extension/module/mpgallery', 'token=' . $this->session->data['token'] . '&module_id=' . $this->request->get['module_id'], true)
			);
		}

		if (!isset($this->request->get['module_id'])) {
			$data['action'] = $this->url->link('extension/module/mpgallery', 'token=' . $this->session->data['token'], true);
		} else {
			$data['action'] = $this->url->link('extension/module/mpgallery', 'token=' . $this->session->data['token'] . '&module_id=' . $this->request->get['module_id'], true);
		}
		...
		if (!empty($this->request->post['gallery'])) {
			$gallerys = $this->request->post['gallery'];
		} elseif (!empty($module_info['gallery'])) {
			$gallerys = $module_info['gallery'];
		} else {
			$gallerys = array();
		}
		...
		if (isset($this->request->post['carousel'])) {
			...
			$data['carousel'] = isset($module_info['carousel']) ? $module_info['carousel'] : '';
		} else {
			$data['carousel'] = '';
		}
		...
		} elseif (!empty($module_info['gall_description'])) {
			$data['gall_description'] = $module_info['gall_description'];
		} else {
			$data['gall_description'] = array();
		}
		...
		$this->response->setOutput($this->load->view('extension/module/mpgallery.tpl', $data));
	}

	protected function validate() {
		if (!$this->user->hasPermission('modify', 'extension/module/mpgallery')) {
